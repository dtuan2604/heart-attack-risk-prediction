...
refs = {
    ...
    "ref.kube-prometheus": "6e5d0145bb69a7cfc9a0c63f60ee7d4640e866e7",
    # renovate: git-refs=https://github.com/kubernetes-monitoring/kubernetes-mixin branch=master
    "ref.kubernetes-mixin": "7e65d914eee7ddb76e596af639c06493ccf48e92",
    # renovate: git-refs=https://github.com/etcd-io/etcd branch=main
    "ref.etcd": "75f1a1dc087bd3cbc68e14a0ddc8ef69db85faa6",
}
...
replacement_map = {
    ...
    "alertmanager-$1": {"replacement": "$1", "init": ""},
    ...
    "(namespace,service)": {"replacement": "(namespace,service,cluster)", "init": ""},
...
def yaml_str_repr(struct, indent=4):
    ...
    text = textwrap.indent(text, " " * indent)[
        indent - 1 :
    ]  # indent everything, and remove very first line extra indentation
    return text
...
def get_rule_group_condition(group_name, value_key):
    ...
    return group_name.replace(
        "Values.defaultRules.rules", f"Values.defaultRules.{value_key}"
    ).strip()
            ...
            while True:
                try:
                    ...
                    while not found_block_end:
                        ...
                        )  # find the starting position of the last line
                        last_line = rules[last_line_index + 1 : next_index]

                        if last_line.startswith("{{- if"):
                            next_index = (
                                last_line_index + 1
                            )  # move next_index back if the current block ends in an if statement
                            continue
...
def add_rules_per_rule_conditions(rules, group, indent=4):
    ...
    for rule in group["rules"]:
        if "alert" in rule:
            rules_condition_map[
                rule["alert"]
            ] = f"not (.Values.defaultRules.disabled.{rule['alert']} | default false)"
...
def add_custom_labels(rules_str, group, indent=4, label_indent=2):
    ...
    rule_group_labels = get_rule_group_condition(
        condition_map.get(group["name"], ""), "additionalRuleGroupLabels"
    )
...
{{- end }}"""
        % (rule_group_labels,),
        " " * (indent + label_indent * 2),
    )

    additional_rule_labels_condition_start = (
        "\n"
        + " " * (indent + label_indent)
        + "{{- if or .Values.defaultRules.additionalRuleLabels %s }}"
        % (rule_group_labels,)
    )
    ...
    ruleStartingLine = [
        (rule_position.start(), rule_position.end())
        for rule_position in rules_positions
    ]
    ...
    for i, rule in enumerate(rules):
        ...
        if current_label:
            ...
            if entries:
                ...
                entries_end = (
                    entries.end() + current_label.end() - section_seperator_len
                )
                rules[i] = (
                    rule[:entries_end]
                    + additional_rule_labels_condition_start
...
def add_custom_annotations(rules, group, indent=4):
    ...
    rule_group_labels = get_rule_group_condition(
        condition_map.get(group["name"], ""), "additionalRuleGroupAnnotations"
    )
    ...
    for alert_position in alerts_positions:
        ...
        index = (
            alert_position.end()
            + annotations_len
            + (rule_condition_len + rule_group_condition_len) * alert
        )
        rules = (
...
def add_custom_alert_rules(rules, key_to_replace, indent):
    ...
    while i < len(rules):
        if rules[i : i + len(alertkey_field)] == alertkey_field:
            ...
            while (
                end_index_alertkey_field < len(rules)
                and rules[end_index_alertkey_field].isalnum()
            ):
        ...
        if found_alert_key:
            if rules[i : i + len(key_to_replace_indented)] == key_to_replace_indented:
                ...
                while (
                    end_index_key_to_replace < len(rules)
                    and rules[end_index_key_to_replace].isalnum()
                ):
                    end_index_key_to_replace += 1

                word_after_key_to_replace = rules[
                    start_index_key_value:end_index_key_to_replace
                ]
                new_key = (
                    key_to_replace_indented
                    + ' {{ dig "'
                    + alertname
                    + '" "'
                    + key_to_replace[:-1]
                    + '" "'
                    + word_after_key_to_replace
                    + '" .Values.customRules }}'
                )
...
def main():
    ...
    for chart in charts:
        if "git" in chart:
            ...
            subprocess.run(
                ["git", "init", "--initial-branch", "main", checkout_dir, "--quiet"]
            )
            subprocess.run(
                ["git", "-C", checkout_dir, "remote", "add", "origin", chart["git"]]
            )
            subprocess.run(
                [
                    "git",
            ...
            if chart.get("mixin"):
                alerts = json.loads(
                    _jsonnet.evaluate_snippet(url, raw_text + ".prometheusAlerts")
                )
            else:
                alerts = yaml.full_load(raw_text)
        ...
        for group in groups:
            write_group_to_file(
                ...
                url,
                chart["destination"],
                chart["min_kubernetes"],
                chart["max_kubernetes"],
            )
