...
charts = [
    ...
    {
        "source": "https://raw.githubusercontent.com/prometheus-operator/kube-prometheus/%s/manifests/grafana-dashboardDefinitions.yaml"
        % (refs["ref.kube-prometheus"],),
        "destination": "../templates/grafana/dashboards-1.14",
        "type": "yaml",
        "min_kubernetes": "1.14.0-0",
        "multicluster_key": ".Values.grafana.sidecar.dashboards.multicluster.global.enabled",
...
def replace_nested_key(data, key, value, replace):
    if isinstance(data, dict):
        return {
            ...
            else replace_nested_key(v, key, value, replace)
...
def patch_json_set_interval_as_variable(content):
    ...
    return re.sub(
        r'"interval"\s*:\s*"(?:\\.|[^\"])*"',
        '"interval":"`}}{{ .Values.grafana.defaultDashboardsInterval }}{{`"',
        content,
        flags=re.IGNORECASE,
    )
...
def main():
    ...
    for chart in charts:
        ...
        if chart["type"] == "yaml":
            ...
            write_group_to_file(
                os.path.basename(source).replace(".json", ""),
                raw_text,
                url,
                chart["destination"],
                chart["min_kubernetes"],
